...
__global__ void RegionPoolForward(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int roi_start_w_in = round(bottom_rois[5] * spatial_scale);
    int roi_start_h_in = round(bottom_rois[6] * spatial_scale);
    int roi_end_w_in = round(bottom_rois[7] * spatial_scale);
    int roi_end_h_in = round(bottom_rois[8] * spatial_scale);
...
template <typename Dtype>
void LoopROIPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
__global__ void RegionPoolBackward(const int nthreads, const Dtype* top_diff,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for (int roi_n = 0; roi_n < num_rois; ++roi_n) {
      ...
      int roi_start_w_in = round(offset_bottom_rois[5] * spatial_scale);
      int roi_start_h_in = round(offset_bottom_rois[6] * spatial_scale);
      int roi_end_w_in = round(offset_bottom_rois[7] * spatial_scale);
      int roi_end_h_in = round(offset_bottom_rois[8] * spatial_scale);
...
template <typename Dtype>
void LoopROIPoolingLayer<Dtype>::Backward_gpu(
...
INSTANTIATE_LAYER_GPU_FUNCS(LoopROIPoolingLayer);
