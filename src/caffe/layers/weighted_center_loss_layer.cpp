...
void WeightedCenterLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void WeightedCenterLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  diff_dims.push_back(num_roi_);
  diff_dims.push_back(dim_);
  diff_.Reshape(diff_dims);
}

template <typename Dtype>
void WeightedCenterLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void WeightedCenterLossLayer<Dtype>::Backward_cpu(
...
#ifdef CPU_ONLY
STUB_GPU(WeightedCenterLossLayer);
#endif

INSTANTIATE_CLASS(WeightedCenterLossLayer);
REGISTER_LAYER_CLASS(WeightedCenterLoss);
